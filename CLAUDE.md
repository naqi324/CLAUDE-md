...
- **mcg-pptx** (`/mcg-pptx`): Creates MCG-branded PowerPoint presentations using the official MCG corporate template. Located at `~/.claude/skills/mcg-pptx/` (-> `~/git/skills/mcg-pptx/`). Self-contained: all 15 scripts bundled in `scripts/`, template in `assets/`, layout catalog in `references/`. Uses the office-skills Python venv.
...
- Use `~/git/system/CLAUDE-md/scripts/audit-mcp-readonly-policy.py` after installing or changing MCP servers to find uncovered safe read-only tools in partially gated namespaces.
...
- **Outlook exception**: `mcp__outlook__*` is fully wildcarded in allow. Destructive ops (`send_email`, `send_draft`, `reply_to_email`, `forward_email`, `delete_email`, `delete_event`, `delete_folder`, `delete_category`, `create_event`, `update_event`, `set_auto_reply`, `respond_event`) are deny-listed and still prompt. Organizational tools (move, rename, create_draft, update flags/categories, add_attachment) and all read-only tools pass through without prompt.
- **Communication MCP deny-verb template**: When adding a new communication MCP (email, chat, calendar), deny these verb patterns: `send_*`, `reply_*`, `forward_*`, `delete_*`, `create_event*`/`create_channel*` (sends invites/notifications), `update_event*` (notifies attendees), `set_auto_reply*`, `respond_event*`. Auto-allow: all `get_*`/`list_*`/`search_*`/`find_*`/`read_*` plus reversible organizational ops (move, rename, create_draft/folder/category, update metadata).
